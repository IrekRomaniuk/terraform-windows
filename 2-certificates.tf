...
resource "azurerm_key_vault" "orion" {
  name                = "${var.prefix}-kvt-a360-net-spoke"
...
resource "azurerm_key_vault_certificate" "orion" {
  name      = "${local.virtual_machine_name}-cert"
  vault_uri = "${azurerm_key_vault.orion.vault_uri}"
